...
class ResidualDenseBlock_out(nn.Module):
    def __init__(self, input, output, nf=c.nf, gc=c.gc, bias=True, use_snorm=False):
        ...
        if use_snorm:
            ...
            # self.s1 = Regularize_block(32, 64, 64, Gaussian_noise=True, Uniform_noise=True)
            # self.s2 = Regularize_block(32, 64, 64, Gaussian_noise=True, Uniform_noise=True)
            # self.s3 = Regularize_block(32, 64, 64, Gaussian_noise=True, Uniform_noise=True)
            # self.s4 = Regularize_block(output, 64, 64, Gaussian_noise=True, Uniform_noise=True)
    ...
    def forward(self, x):
        ...
        # x2 = self.s1(self.lrelu(self.conv2(torch.cat((x, x1), 1))))
        # x3 = self.s2(self.lrelu(self.conv3(torch.cat((x, x1, x2), 1))))
        # x4 = self.s3(self.lrelu(self.conv4(torch.cat((x, x1, x2, x3), 1))))
