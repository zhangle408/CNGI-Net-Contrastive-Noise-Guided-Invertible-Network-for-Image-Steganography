...
transforms_stego = transforms_color
...
test_v_dataset_secret = ImageFolder(
            c.secretdir,
            transforms_secret)
test_v_dataset_stego = ImageFolder(
            c.stegodir,
            transforms_stego)
test_v_loader_secret = DataLoader(test_v_dataset_secret, batch_size=c.batch_size,
                                        shuffle=True, num_workers=1)
test_v_loader_cover = DataLoader(test_v_dataset_cover, batch_size=c.batchsize_val,
                                       shuffle=True, num_workers=1)
test_v_loader_stego = DataLoader(test_v_dataset_stego, batch_size=c.batchsize_val,
                                       shuffle=True, num_workers=1)
test_v_loader = zip(test_v_loader_secret, test_v_loader_cover, test_v_loader_stego)
